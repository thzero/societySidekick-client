...
const storeFactions = {
	...
	actions: {
		async getFactionListing(correlationId, gameSystemId) {
			...
			const service = LibraryClientUtility.$injector.getService(AppConstants.InjectorKeys.SERVICE_FACTIONS);
			const response = await service.listing(correlationId, gameSystemId);
			this.$logger.debug('store.factions', 'setFactionListing', 'response', response, correlationId);
			if (Response.hasSucceeded(response)) {
				const listing = response.results ? response.results.data : null;
				setFactionListing(correlationId, listing);
				return listing;
			}
			return [];
		},
	...
	dispatcher: {
		async getFactionListing(correlationId, gameSystemId) {
			return await LibraryClientUtility.factions.$store.getFactionListing(correlationId, gameSystemId);
		}
	}
};

export default storeFactions;
