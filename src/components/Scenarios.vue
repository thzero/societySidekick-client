...
export default {
	...
	setup(props, context) {
		...
		const externalListType = ref(Constants.ExternalListTypes.Scenarios);
		...
		const fetch = async (correlationIdI) => {
			...
			try {
				...
				logger.debug('Scenarios', 'fetch', 'response', responseUser, correlationIdI);
				...
				logger.debug('Scenarios', 'fetch', 'user', user.value);

				const responseCharacter = await serviceCharacters.listingByShortId(correlationIdI, gamerTag, gameSystemId.value);
				logger.debug('Scenarios', 'fetch', 'response', responseCharacter, correlationIdI);
				...
				logger.debug('Scenarios', 'fetch', 'characters', charactersValue, correlationIdI);
				characters.value = charactersValue;

				// scenarioListRef.value.execute()
			}
			finally {
