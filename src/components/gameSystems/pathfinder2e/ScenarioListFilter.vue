	...
	computed: {
		scenarioAdventureFilter: {
			get: function () {
				...
				return AppUtility.settings().getSettingsUserScenariosGameSystem(this.correlationId(), this.$store.state.user.user, SharedConstants.GameSystems.Pathfinder2e.id, (settings) => settings.scenarioAdventureFilter);
			},
			set: function (newVal) {
				if (this.externalList) {
				...
				AppUtility.settings().updateSettingsUserScenariosGameSystem(this.correlationId(), this.$store, this.$store.state.user.user, SharedConstants.GameSystems.Pathfinder2e.id, newVal, (settings) => { return settings.scenarioAdventureFilter = newVal; });
			}
		},
		scenarioAdventures() {
