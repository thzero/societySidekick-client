<template>
	<v-card
		:class="'pl-2 pr-2 mb-2 ' + ($vuetify.display.lgAndUp ? 'ml-1 mr-1' : '')"
	>
		...
		<v-card-text>
			...
			>
				...
				<!--eslint-enable-->
				<div v-if="boonUses(value.boonId)">
					<table><tbody>
						<tr>
							<td>
								<span class="subtitle-2">{{ $t('characters.gameSystems.starfinder1e.boons.uses') }}:</span>
							</td>
							<td
		...
		<v-card-actions v-if="!hasScenario">
			<v-spacer />
			<v-btn
				color="primary"
				variant="text"
...
import { useGameSystemBaseBoonComponent } from '@/components/gameSystems/baseBoon';
...
export default {
	...
	setup(props, context) {
		...
		const base = useGameSystemBaseBoonComponent(props, context, { serviceGameSystem, rulesGameSystem });
