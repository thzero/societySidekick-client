...
export default {
	...
	created() {
		const self = this;
		this.$EventBus.$on('initializeCompleted', (value) => {
			self.initializeCompleted = value;
		});
	},
	methods: {
		getSettingsUserTab(user, funcAttribute) {
	...
	async beforeRouteEnter (to, from, next) {
		...
		(async () => {
			try {
				Vue.prototype.$EventBus.$emit('initializeCompleted', false);

				await Promise.all([
					Vue.prototype.$store.dispatcher.news.getLatest(),
					Vue.prototype.$store.dispatcher.characters.getCharacterListing({ listing: true })
				]);
			}
			finally {
				const timeout = setTimeout(function () {
					Vue.prototype.$EventBus.$emit('initializeCompleted', true);
	...
	async beforeRouteUpdate (to, from, next) {
		...
		(async () => {
			...
			try {
				...
				await Promise.all([
					this.$store.dispatcher.news.getLatest(),
					this.$store.dispatcher.characters.getCharacterListing({ listing: true })
				]);
			}
			finally {
				const timeout = setTimeout(function () {
